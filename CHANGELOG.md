...
- Navigation is now predictable: everything you can act on — a home theater, a speaker group, or a single room — opens as a full page, and sheets are reserved for read-only peeks (the profile's captured-entity view). The guided home-theater and group-creation flows now cover the bottom tabs so you can't accidentally tab away mid-setup.
- Polish on the detail pages: the room diagram no longer blows up to fill a wide desktop window, the "Separate" button always sits at the bottom of the page, the home-theater "Configure" button is shorter, and a profile's "Re-capture from current setup" action moved above the captured list.
