...
### Fixed
- Switching Trueplay on for a home theater or pair while some of its speakers have no stored tuning now asks first and says what it could cost. Sonos applies a tuning for the bonded set as a whole, so doing it can destroy the tunings the other speakers still have, permanently. That is the normal state right after adding a speaker, which is exactly when the switch gets reached for. Switching it off doesn't ask.
