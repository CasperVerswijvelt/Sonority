...
- Creating a speaker group from a speaker that's bonded elsewhere now frees it first. Sonos accepts the grouping command and silently does nothing otherwise, so the group would never have formed. The “Group with another speaker” shortcut on a room page counts those speakers as candidates too, so it no longer hides itself on a system whose only other speaker is bonded.
...
- The Trueplay row breaks its count down per speaker when they disagree, so a home theater reading "5/6 tuned" names the one that isn't. Each row gives the model and the channel ("One SL · Surround R"), so matched surrounds are told apart (two subwoofers still read alike; Identify tells those apart). A speaker Sonority couldn't reach is listed as "Couldn't read" instead of vanishing from the count, one it is still reading says "Checking…" rather than claiming either, and a set with any unread speaker is no longer called untuned. The breakdown re-reads when the home theater's speakers change.
...
### Changed
- The Trueplay row counts tuned speakers before active ones, so a set with stored tunings that are switched off no longer opens with "0/6 active" and reads as though nothing were tuned.
...
- The home theater review step now names what the apply costs, in one warning card: the speakers that leave, and the speakers that lose their Trueplay.
- The speaker group review step now names the speakers that lose their Trueplay, so the cost is stated on the screen with the Apply button. Grouping can't absorb a speaker out of another bond, which makes it the more destructive path, and the warning previously appeared only three steps earlier under the speaker list.
...
- Applying a profile now asks first when it re-bonds anything, naming the speakers that could lose their Trueplay tuning. An unchanged re-apply writes nothing, so it still applies straight through without a dialog.
- Separating a home theater or a speaker group, and removing speakers from a home theater, now name the speakers that could lose their Trueplay tuning. A removal clears the tuning of the whole bonded set, not just what leaves; an untuned set is told nothing.
...
### Fixed
- The home theater, speaker group and room pages no longer stretch edge to edge on a tablet or a wide desktop window. They now clamp to a readable column like the setup flows already did, so a speaker's name and its Trueplay state stay next to each other instead of a screen apart. Phone layout is unchanged.
...
- Pairing or grouping two speakers that both come from the same group now breaks that group up once instead of sending a second, pointless command against a setup that no longer exists.
...
- Bonding a tuned stereo pair as dedicated fronts no longer separates the pair first, which was an unnecessary extra teardown. Expect to re-tune either way: Sonos switches Trueplay off across the bonded set, and switching it back on may clear what is left.
- The speaker list and the review step no longer disagree about which speakers lose their Trueplay: both are now computed the same way, so the warning under the list can't stay silent while the heading above it says the tuning is cleared, and a speaker whose Trueplay could not be read is now listed as at risk instead of being quietly left out.
