...
### Fixed
- Speaker pickers no longer title a speaker that's already in the home theater or group you're configuring by its room name: Sonos gives every bonded speaker the bond's name, so both front speakers showed up as two cards reading "Living Room". They're now labelled by speaker type, like the other bonded speakers in the list.
- Turning Trueplay on or off for a home theater or pair now asks first when some of its speakers have no stored tuning. Sonos applies a tuning for the bonded set as a whole, so turning it on can destroy the tunings the other speakers still have, permanently, and turning it off means you may not be able to turn it back on. That partly-tuned state is the normal one right after adding a speaker, which is exactly when the switch gets reached for. A fully tuned set never asks.
