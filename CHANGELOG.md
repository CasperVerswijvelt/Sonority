...
- Speaker groups now use Sonos' newer local zone API when the household supports it: creating, editing and separating a group each apply in a single call instead of a retry loop or a dissolve-and-rebuild, so changes land faster and more reliably. Removing a speaker from a group no longer tears the group down, and the freed speaker keeps its room name. Every path falls back to the previous method automatically. Home theatres deliberately keep the existing path, which preserves a Trueplay tuning.
