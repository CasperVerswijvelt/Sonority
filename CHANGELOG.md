...
- The Trueplay row now breaks its count down per speaker when they disagree, so a home theater reading "5/6 tuned" names the speaker that isn't instead of leaving you to guess. Each row names it by model and by the channel it holds ("One SL · Surround R"), so two matched surrounds are told apart — two subwoofers in one home theater are the one pair that still reads alike, and Identify tells those apart. A speaker Sonority couldn't reach is listed as "Couldn't read" rather than dropping out of the count with nothing on screen to say which speaker it was (unless it was read successfully earlier in the session, in which case that older reading is still shown), and a set where any speaker couldn't be read no longer claims the speakers are untuned.
...
- Bonding a tuned stereo pair as dedicated fronts no longer separates the pair first, which was an unnecessary extra teardown. The stored tuning survives the bond where it previously did not, though Sonos still switches Trueplay off and switching it back on may clear it, so re-tuning is still needed either way.
