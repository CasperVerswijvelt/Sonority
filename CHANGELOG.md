...
### Fixed
- Discovery no longer trusts a stale address: if a speaker's description URL answers as a different player, that answer is discarded instead of being filed under the wrong speaker. A speaker the topology gives no address for at all is now recorded in the diagnostics log rather than quietly skipped.
