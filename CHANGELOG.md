...
### Added
- **EQ** — a 10-band equaliser for any home theater, speaker group or single speaker, applied to the speakers themselves; one curve for the whole configuration or a separate curve per speaker, with optional live apply. Something the Sonos app doesn't offer at all. ⚠️ A speaker holds only one tuning, so applying an EQ permanently replaces a Trueplay calibration measured in the Sonos app — Sonority asks first. A room-measurement step is shown as coming soon; the sliders are already offsets on top of it, so nothing you set now is wasted.
